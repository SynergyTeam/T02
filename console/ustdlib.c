...
//*****************************************************************************
//XXX
//! Copies a characters from one string to another.
//!
...
//*****************************************************************************
char *
ustrcpy(char *restrict s1, const char *restrict s2)
{
  char *s;
...
/*
 * XXX copy char s2[] to end of s1[]
 */
char *
ustrcat(char *restrict s1, const char *restrict s2)
{
	char *s;
...
/*
 * XXX copy char s2[max n] to end of s1[]
 */
char *
ustrncat(char *restrict s1, const char *restrict s2, int n)
{
	char *s;
...
uvsnprintf(char * restrict s, size_t n, const char * restrict format,
           va_list arg)
{
    unsigned long ulIdx, ulValue, ulCount, ulBase, ulNeg, upCase;
    char *pcStr, cFill;
    int iConvertCount = 0;
...
again:
            ...
            switch(*format++)
            {
                ...
                case 's':
                {
                    ...
                    {
                        //XXX Use certain length of string
                        if((ulCount != 0) && (ulIdx >= ulCount))
                            break;
                    }
...
//*****************************************************************************
unsigned long
ustrtoul(const char *restrict nptr, char **restrict endptr, int base)
{
    ...
    //
    assert_param(nptr);
    assert_param((base == 0) || ((base > 1) && (base <= 16)));
    ...
    if(endptr)
    {
        *endptr = ulValid ? (char*)pcPtr : (char*)nptr;
    }
