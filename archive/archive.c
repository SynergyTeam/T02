...
//#pragma FUNCTION_OPTIONS (ArchiveClean, "--opt_level=0")                      //FIXME опция выдает ошибку на данной версии компилятора
...
static int32_t checking_border(archive_str *arch, int nSize) {
	...
	while (nLData == (arch->bData & ~FLASH_BLOCK_MASK)) {
        ...
        for (dlen = USEFUL_DATA; dlen > 0; ) {
            ...
            // Проверка размера пакета. XXX Возможно лучшей вариант проверять целостность структуры
            if (plen < MIN_ARCH_SIZE || plen > MAX_ARCH_SIZE) {
